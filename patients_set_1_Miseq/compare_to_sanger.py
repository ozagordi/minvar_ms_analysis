...
# Sanger results
sanger_file = '/data/MinVar/Patients_Sanger_Sequencing/PatientSamples_1-10.tsv'
with open(sanger_file) as csvfile:
    pat_list = csv.DictReader(csvfile, delimiter='\t')
    for r in pat_list:
        if r['Patient'] == sys.argv[1]:
            sanger_muts_protease = {}
            sanger_muts_RT = {}
            for m in r['Sanger Mutationen PR'].split(','):
                try:
                    pos, mut = int(m[:-1]), m[-1]
                except ValueError:
                    continue
            ...
            for m in r['Sanger Mutationen RT'].split(','):
                pos, mut = int(m[:-1]), m[-1]
                try:
