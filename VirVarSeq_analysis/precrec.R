...
assert_that(length(virvarfile) == 1, silent=TRUE)
...
pr = single_prec_rec(df_results)
ofn = paste0('summary/precrec_', mix_number, '.csv')
data.frame(mix=mix_number,precision=pr[1],recall=pr[2]) %>%
  write.table(file=ofn, sep="\t", row.names=FALSE)
